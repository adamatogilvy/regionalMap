...
Snap.load('images/usaLow.svg', function (response) {
    ...
    $('#state-selection').change(function() {
        ...
        var regionNumber = region.slice(1);
       ...
       var activeRegion = Snap.select('#'+region);
        activeRegion.unhover().animate({
            opacity:1,
            fill: '#eeeadf'
        },300);
        //animate the circle marker
        activeRegion.select('#marker-'+regionNumber).attr({
            fill: '#ed5155'
        });
        //show territories
        var terrTest = activeRegion.select('#territories-'+regionNumber);
        if(terrTest != null) {
            terrTest.animate({
                opacity: 1
            },300);
